...
const typeCheck = (args, checker) => {
  ...
  if (isString(checker)) {
    if (!isTypeOf(args, checker))
      throw new TypeError(
        `Arg type mismatch. Required ${checker}, found ${args}.`
      );
    ...
    if (args.length !== checker.length)
      throw new TypeError(
        `Arg length mismatch. Required ${args.length}, found ${checker.length} args in ${args}.`
      );
  ...
  window.unregisterHMSEvent = (eventName, handler) => {
    if (hmsEventHandlers.hasOwnProperty(eventName)) {
      if (handler) {
        ...
        // if no specific handler is specified, remove all
...
const initConstantSetter = () => {
  ...
  window.hmsSetConstants = (objName, constants) => {
    console.log(
      `${TAG} :: initializing constants for`,
      objName,
      "with",
      constants
    );
...
// with `cb` prefix.
const generateFunctionVariations = (
  exportedFunctions,
  carrierObj,
  javaClassName
) => {
